...
AI client: OpenAI-compatible interface (works with Anthropic, OpenAI, DeepSeek, Ollama/LiteLLM via `LLM_BASE_URL`/`LLM_MODEL`/`LLM_PROVIDER`/`LLM_TEMPERATURE`). 8s timeout, 2 retries exponential backoff.
...
- **Learning**: on success, store `(fingerprint_pattern, actions)` in SQLite; hourly job promotes patterns with ≥5 successes to Tier1 YAML; `/admin/patterns` (basic-auth) to view.
...
## Deployment
Dockerfile: multi-stage `golang:1.21-alpine` build (CGO_ENABLED=0) → `alpine:3.19` runtime, non-root, <50MB, `ENTRYPOINT ["/sora"]`, default `CMD ["--mode=full","--config=/etc/sora/config.yaml"]`.
K8s: 2+ replicas, requests 100m/128Mi, limits 500m/512Mi, liveness `/health`@15s, readiness `/ready`@10s (initial delay 5s), rolling update maxUnavailable:0. HPA min2/max10 on CPU>70%.
Helm: `values.yaml` exposes image/replicas/mode/resources/config/secrets; templates use `_helpers.tpl` for name/labels.
